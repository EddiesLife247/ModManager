...
module.exports = async (client, oldChannel, newChannel) => {
    try {
        client.logchannel = botsql.prepare(`SELECT logchannel FROM settings WHERE guildid = '${oldChannel.guild.id}'`);
        if (client.logchannel.all().length) {
            const logchannel = oldChannel.guild.channels.cache.get(client.logchannel.get().logchannel);
            ...
            if (oldChannel.guild.members.me.permissions.has(PermissionsBitField.Flags.ViewAuditLog)) {
                try {
                    ...
                    console.log(err);
                    client.guilds.cache.get("787871047139328000").channels.cache.get("901905815810760764").send({ content: `channelUpdate.js : An Error occured: ${err}` });
                    return;
                }
            }
        }
        ...
        console.log(e);
        client.guilds.cache.get("787871047139328000").channels.cache.get("901905815810760764").send({ content: `channelUpdate.js : An Error occured: ${e}` });
    }
