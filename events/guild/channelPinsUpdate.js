...
module.exports = async (client, channel) => {
    try {
        ...
        if (client.logchannel.all().length) {
            ...
            if (channel.guild.members.me.permissions.has(PermissionsBitField.Flags.ViewAuditLog)) {
                try {
                    ...
                    console.log(err);
                    client.guilds.cache.get("787871047139328000").channels.cache.get("901905815810760764").send({ content: `channelPinsUpdate.js : An Error occured: ${err}` });
                    return;
                }
            }
        }
        ...
        console.log(e);
        client.guilds.cache.get("787871047139328000").channels.cache.get("901905815810760764").send({ content: `channelPinsUpdate.js : An Error occured: ${e}` });
    }
