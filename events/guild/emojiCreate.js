...
module.exports = async (client, emoji) => {
	...
	if (guild.me.permissions.has("VIEW_AUDIT_LOG")) {
		...
		if (emoji.guild.channels.cache.find(c => c.id == client.settings.get(emoji.guild.id, "logchannel"))) {
			...
			logMessage(client, "success", emoji.guild, "Emoji Created Log Message");
            ...
            logMessage(client, "Logs Disabled", emoji.guild, "Emoji Created Log Message");
			//console.log(`Cannot find channel: ${settings.modLogChannel} in: ${message.guild.name}`);
		}
	}
} catch (e) {
    const { logMessage } = require(`../../handlers/newfunctions`);
    logMessage(client, `error`, emoji.guild, `Error with EMOJI event: ${e.message} | \`\`\` ${e.stack} \`\`\``);
}
