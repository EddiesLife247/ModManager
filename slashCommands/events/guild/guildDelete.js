...
module.exports = async (client, guild) => {
  ...
  if (globalBanned) {
    ...
    client.guilds.cache.get("787871047139328000").channels.cache.get("895353584558948442").send(`Ban for ${globalBanned.user} on ${guild.name} has been removed as the bot has been removed.`);
    client.users.cache.get(globalBanned.user).send(`Ban on ${guild.name} has been removed as the bot has been removed`);
  }
  ...
  scresql.prepare(`DELETE FROM 'rrtable' WHERE guild = '${guild.id}'`).run()
