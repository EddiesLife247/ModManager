...
module.exports = (client) => {
  ...
  function receiveQueueData(newQueue, newTrack) {
    ...
    var embed = new MessageEmbed().setColor(ee.color)
      ...
      .addField(`↪️ Random Playlist:`, `>>> ${newQueue.autoplay ? `${client.allEmojis.check_mark}` : `${client.allEmojis.x}`}`, true)
    ...
    let stop = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji(`🏠`).setLabel(`Leave`)
    let pause = new MessageButton().setStyle('SECONDARY').setCustomId('3').setEmoji('⏸').setLabel(`Pause`)
    let autoplay = new MessageButton().setStyle('SUCCESS').setCustomId('4').setEmoji('🔁').setLabel(`Random Playlist`)
    ...
    const row2 = new MessageActionRow().addComponents([songloop, queueloop, rewind, forward]);
    return {
      embeds: [embed],
      components: [row, row2]
    };
