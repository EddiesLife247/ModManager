...
module.exports.msgCooldown = msgCooldown;
module.exports.formatDate = formatDate;
...
function msgCooldown(message, cooldown) {
  ...
  if (cooldown2 > 0) {
    ...
    if (!client.msgcooldowns.has(message.member.id)) {
      ...
      client.msgcooldowns.set(message.member.id, new Collection());
    }
...
function change_status(client) {
  try {
    client.user.setActivity(`${Math.ceil(client.users.cache.size / 1000)}k Users | ${config.prefix}help `, {
      type: "WATCHING",
    });
  } catch (e) {
    console.log(String(e.stack).bgRed)
